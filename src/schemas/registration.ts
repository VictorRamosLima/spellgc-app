import * as yup from 'yup'

const phoneRegExp = /^\d{4,5}-?\d{4}$/
const dddRegExp = /^\d{2}$/
const cvvRegExp = /^\d{3}$/
const cardNumberRegExp = /^\d{16}$/
const cepRegExp = /^\d{5}-?\d{3}$/
const cpfRegExp = /^\d{3}[\w\-._]?\d{3}[\w\-._]?\d{3}[\w\-._]?\d{2}$/
const passwordRegExp = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/

import {
  genders,
  states,
  publicPlaceTypes,
  homeTypes,
  telephoneTypes,
  countries,
  flags,
} from '../utils'

export const addressSchema = yup.object().shape({
  publicPlace: yup
    .string()
    .required('Logradouro é obrigatório'),
  publicPlaceType: yup
    .string()
    .oneOf(Object.keys(publicPlaceTypes), 'Tipo do logradouro é obrigatório'),
  homeType: yup
    .string()
    .oneOf(Object.keys(homeTypes), 'Tipo de residência é obrigatório'),
  zipCode: yup.string().required('CEP é obrigatório').matches(cepRegExp, 'CEP é inválido'),
  number: yup.number().typeError('Número é inválido').required('Número é obrigatório'),
  complement: yup.string(),
  neighborhood: yup.string().required('Bairro é obrigatório'),
  city: yup.string().required('Cidade é obrigatória'),
  country: yup.string().oneOf(countries, 'País é obrigatório'),
  state: yup.string().oneOf(Object.keys(states), 'Estado é obrigatório'),
})

export const creditCardSchema = yup.object().shape({
  number: yup
    .string()
    .required('Número do cartão é obrigatório')
    .matches(cardNumberRegExp, 'Número do cartão é inválido'),
  cvv: yup.string().required('CVV é obrigatório').matches(cvvRegExp, 'CVV é inválido'),
  flag: yup.string().oneOf(Object.keys(flags), 'Bandeira é obrigatória'),
  name: yup.string().required('Nome impresso é obrigatório'),
})

export const registrationSchema = yup.object().shape({
  name: yup.string().required('Nome é obrigatório'),
  gender: yup.string().oneOf(Object.keys(genders), 'Gênero é obrigatório'),
  birthdate: yup
    .string()
    .required('Data de nascimento é obrigatória')
    .test(
      'birthdate',
      'Deve ter 18 anos ou mais',
      value => Math.floor((new Date().getTime() - new Date(value).getTime()) / (365.25 * 24 * 60 * 60 * 1000)) >= 18
    ),
  email: yup.string().required('E-mail é obrigatório').email('E-mail é inválido'),
  cpf: yup.string().required('CPF é obrigatório').matches(cpfRegExp, 'CPF é inválido'),
  ddd: yup.string().required('DDD é obrigatório').matches(dddRegExp, 'DDD é inválido'),
  telephoneType: yup
    .string()
    .oneOf(Object.keys(telephoneTypes), 'Tipo de telefone é obrigatório'),
  telephone: yup
    .string()
    .required('Telefone é obrigatório')
    .matches(phoneRegExp, 'Telefone é inválido'),
  password: yup
    .string()
    .test('len', 'Senha deve conter 8 ou mais caracteres', val => val.length >= 5)
    .matches(
      passwordRegExp,
      'Senha deve ter letras maiúsculas, minúsculas e ao menos um caractere especial'
    )
    .required('Senha é obrigatória'),
  passwordConfirmation: yup
    .string()
    .required('Confirmação de senha é obrigatória')
    .oneOf([yup.ref('password')], 'Senhas devem ser iguais'),
  billingAddress: addressSchema,
  shippingAddress: addressSchema,
  creditCard: creditCardSchema,
})

